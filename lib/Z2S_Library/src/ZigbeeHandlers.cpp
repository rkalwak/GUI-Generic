...
[[maybe_unused]] static esp_err_t zb_action_handler(esp_zb_core_action_callback_id_t callback_id, const void *message) {
  ...
  switch (callback_id) {
    ...
    case ESP_ZB_CORE_CMD_CUSTOM_CLUSTER_REQ_CB_ID:
      ret = zb_cmd_custom_cluster_req_handler((esp_zb_zcl_custom_cluster_command_message_t *)message);
      break;
...
static bool zb_raw_cmd_handler(uint8_t bufid) {
  ...
  log_i("is common comand %d, disable_default_response %d, is_manuf_specific %d", cmd_info->is_common_command, cmd_info->disable_default_response,
  ...
  for (std::list<ZigbeeEP *>::iterator it = Zigbee.ep_objects.begin(); it != Zigbee.ep_objects.end(); ++it) {
    if (cmd_info->addr_data.common_data.dst_endpoint == (*it)->getEndpoint()) {
      if ((*it)->zbRawCmdHandler(esp_zb_zcl_address, cmd_info->addr_data.common_data.src_endpoint, cmd_info->addr_data.common_data.dst_endpoint,
                                 cmd_info->cluster_id, cmd_info->cmd_id, cmd_info->is_common_command, cmd_info->disable_default_response,
                                 cmd_info->is_manuf_specific, cmd_info->manuf_specific, sizeof(buf), &buf[0])) {
...
static esp_err_t zb_attribute_set_handler(const esp_zb_zcl_set_attr_value_message_t *message) {
  ...
  log_v("Received attribute set message: endpoint(%d), cluster(0x%x), attribute(0x%x), data size(%d)", message->info.dst_endpoint,
...
static esp_err_t zb_attribute_reporting_handler(const esp_zb_zcl_report_attr_message_t *message) {
  ...
  log_v("Received report from address(0x%x) src endpoint(%d) to dst endpoint(%d) cluster(0x%x)", message->src_address.u.short_addr,
        message->src_endpoint, message->dst_endpoint, message->cluster);
  // List through all Zigbee EPs and call the callback function, with the message
  for (std::list<ZigbeeEP *>::iterator it = Zigbee.ep_objects.begin(); it != Zigbee.ep_objects.end(); ++it) {
    if (message->dst_endpoint == (*it)->getEndpoint()) {
      (*it)->zbAttributeReporting(message->src_address, message->src_endpoint, message->cluster,
...
static esp_err_t zb_cmd_read_attr_resp_handler(const esp_zb_zcl_cmd_read_attr_resp_message_t *message) {
  ...
  log_v("Read attribute response: from address(0x%x) src endpoint(%d) to dst endpoint(%d) cluster(0x%x)", message->info.src_address.u.short_addr,
  ...
  for (std::list<ZigbeeEP *>::iterator it = Zigbee.ep_objects.begin(); it != Zigbee.ep_objects.end(); ++it) {
    if (message->info.dst_endpoint == (*it)->getEndpoint()) {
      ...
      while (variable) {
        log_v("Read attribute response: status(%d), cluster(0x%x), attribute(0x%x), type(0x%x), value(%d)", variable->status, message->info.cluster,
        ...
        if (variable->status == ESP_ZB_ZCL_STATUS_SUCCESS) {
          if (message->info.cluster == ESP_ZB_ZCL_CLUSTER_ID_BASIC) {
            ...
            (*it)->zbAttrReadResponse(message->info.src_address, message->info.src_endpoint, message->info.cluster,
...
static esp_err_t zb_configure_report_resp_handler(const esp_zb_zcl_cmd_config_report_resp_message_t *message) {
  ...
  for (std::list<ZigbeeEP *>::iterator it = Zigbee.ep_objects.begin(); it != Zigbee.ep_objects.end(); ++it) {
    if (message->info.dst_endpoint == (*it)->getEndpoint()) {
      ...
      while (variable) {
        log_v("Configure report response: status(%d), cluster(0x%x), direction(0x%x), attribute(0x%x)", variable->status, message->info.cluster,
              variable->direction, variable->attribute_id);
        (*it)->zbConfigReportResponse(message->info.src_address, message->info.src_endpoint, message->info.cluster, variable->status,
                                      variable->direction, variable->attribute_id);
        variable = variable->next;
      }
    }
...
static esp_err_t zb_cmd_default_resp_handler(const esp_zb_zcl_cmd_default_resp_message_t *message) {
  ...
  log_v("Received default response: from address(0x%x), src_endpoint(%d) to dst_endpoint(%d), cluster(0x%x) for command (0x%x) with status 0x%x",
        message->info.src_address.u.short_addr, message->info.src_endpoint, message->info.dst_endpoint, message->info.cluster, message->resp_to_cmd,
        message->status_code);
  log_v("command id (%d), direction (%d), is common (%d)", message->info.command.id, message->info.command.direction,
        message->info.command.is_common);
  for (std::list<ZigbeeEP *>::iterator it = Zigbee.ep_objects.begin(); it != Zigbee.ep_objects.end(); ++it) {
    if (message->info.dst_endpoint == (*it)->getEndpoint()) {
      (*it)->zbCmdDefaultResponse(message->info.src_address, message->info.src_endpoint, message->info.cluster, message->resp_to_cmd,
                                  message->status_code);
    }
  }
  return ESP_OK;
...
static esp_err_t zb_cmd_ias_zone_status_change_handler(const esp_zb_zcl_ias_zone_status_change_notification_message_t *message) {
  ...
  log_v(
      "IAS Zone Status Notification: from address(0x%x) src endpoint(%d) to dst endpoint(%d) cluster(0x%x), zone status (0x%x), extended status "
      "(0x%x), zone id (0x%x), delay (0x%x)",
      message->info.src_address.u.short_addr, message->info.src_endpoint, message->info.dst_endpoint, message->info.cluster, message->zone_status,
  ...
  for (std::list<ZigbeeEP *>::iterator it = Zigbee.ep_objects.begin(); it != Zigbee.ep_objects.end(); ++it) {
    if (message->info.dst_endpoint == (*it)->getEndpoint()) {
      ...
      while (variable) {
        ...
        // message->info.src_address.u.short_addr, message->info.src_endpoint, message->info.cluster, variable->attr_id, variable->data_type);
...
static esp_err_t zb_cmd_custom_cluster_req_handler(esp_zb_zcl_custom_cluster_command_message_t *message) {
  ...
  for (uint8_t i = 0; i < message->data.size; i++) log_i("Payload [0x%x] = 0x%x", i, *(((uint8_t *)(message->data.value)) + i));

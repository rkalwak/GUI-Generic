...
OneWireBus::OneWireBus(uint8_t pinNumberConfig) : oneWire(pinNumberConfig), pin(pinNumberConfig), nextBus(nullptr), lastReadTime(0) {
  ...
  supla_log(LOG_DEBUG, "OneWire(pin %d) Found %d devices:", pinNumberConfig, sensors.getDeviceCount());
...
void DS18B20::iterateAlways() {
  ...
  unsigned long timeSinceLastRead = currentTime - myBus->lastReadTime;
  ...
  if (timeSinceLastRead >= conversionInterval) {
    ...
    myBus->lastReadTime = currentTime;
  }

  if (timeSinceLastRead >= updateInterval && (lastReadTime != myBus->lastReadTime)) {
